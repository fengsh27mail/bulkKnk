...
test_that("run_virtual_knockout basic knockout works", {
  ...
  # A should be 0 (use [[ ]] for unnamed scalar comparison)
...
test_that("run_virtual_knockout with NULL target returns steady state", {
  genes <- c("X", "Y", "Z")
...
test_that("run_virtual_knockout rejects invalid inputs", {
  ...
  # Non-matrix adj_matrix
  expect_error(run_virtual_knockout(E_init = E, adj_matrix = "not_a_matrix"),
               "matrix")
